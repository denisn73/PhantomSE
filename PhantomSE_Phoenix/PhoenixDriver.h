...
//Init
//--------------------------------------------------------------------
bool ServoDriver::Init(void) {
  bool servoInited = false;
  if(ax12InitServo(120)) servoInited = true;
  g_fServosFree = true;
...
//--------------------------------------------------------------------
#ifdef cVoltagePin
word  g_awVoltages[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
...
word ServoDriver::GetBatteryVoltage(void) {
  ...
  if (g_wLastVoltage != 0xffff) {
    if ( (ulDeltaTime < VOLTAGE_MIN_TIME_BETWEEN_CALLS) || (bioloid.interpolating &&  (ulDeltaTime < VOLTAGE_MAX_TIME_BETWEEN_CALLS))) return g_wLastVoltage;
  }
  // Lets cycle through the Tibia servos asking for voltages as they may be the ones doing the most work...
  register word wVoltage = 120;
  ...
  if (ulDeltaTime < VOLTAGE_TIME_TO_ERROR)  return g_wLastVoltage;
  return 0;
}
#endif
...
boolean ServoDriver::FIsGPSeqDefined(uint8_t iSeq) {
  ...
  if (iSeq >= GPSEQ_EEPROM_MAX_SEQ) return false;
  ...
  if ((g_wSeqHeaderStart < GPSEQ_EEPROM_START_DATA) || (g_wSeqHeaderStart >= GPSEQ_EEPROM_SIZE)) return false;  // pointer does not look right.
...
void ServoDriver::GPStartSeq(uint8_t iSeq) {
  ...
  if (!FIsGPSeqDefined(iSeq)) return;
  _fGPActive = true;
  _iSeq = iSeq;
...
void ServoDriver::GPPlayer(void) {
  ...
  if (_fGPActive) {
    ...
    #ifdef USE_PYPOSE_HEADER
    if (g_fSeqProgmem) {
      if (_sGPSM >= 0) g_ptransCur++;  // lets point to the next step entry.
      else g_ptransCur--;  // lets point to the next step entry.
      ...
      if (!fRobotUpsideDownGPStart) {
        // set a servo value by index for next pose
        for(bServo = 0; bServo < poseSize; bServo++) bioloid.setNextPoseByIndex(bServo, pgm_read_word_near(pwPose + 1 + bServo)); 
      } else {
        // Upside down
        for (bServo = 0; bServo < poseSize; bServo++) {
          bServoIndexUpsideDown = pgm_read_byte(&cPinIndexTranslateUpsideDownTable[bServo]);
          if (bServoIndexUpsideDown & 0x80) bioloid.setNextPoseByIndex(bServoIndexUpsideDown & 0x7f, 1023 - pgm_read_word_near(pwPose + 1 + bServo));
          else bioloid.setNextPoseByIndex(bServoIndexUpsideDown, pgm_read_word_near(pwPose + 1 + bServo));
        }
      }
    ...
    for (bServo = 0; bServo < NUMSERVOS; bServo++) {
      ...
      if (!fRobotUpsideDownGPStart) bioloid.setNextPoseByIndex(bServo, wPosePos);  // set a servo value by index for next pose
      else {
        bServoIndexUpsideDown = pgm_read_byte(&cPinIndexTranslateUpsideDownTable[bServo]);
        if (bServoIndexUpsideDown & 0x80) bioloid.setNextPoseByIndex(bServoIndexUpsideDown & 0x7f, 1023 - wPosePos);
        else bioloid.setNextPoseByIndex(bServoIndexUpsideDown, wPosePos);
      }
...
//------------------------------------------------------------------------------------------
// How many steps does the current sequence have
//------------------------------------------------------------------------------------------
uint8_t ServoDriver::GPNumSteps(void) { return _fGPActive ? g_eepph.bCntSteps : 0; }

//------------------------------------------------------------------------------------------
// Return which step currently on...
//------------------------------------------------------------------------------------------
uint8_t ServoDriver::GPCurStep(void) { return _fGPActive ? g_bSeqStepNum + 1 : 0xff; }

//------------------------------------------------------------------------------------------
// Set the Speed multiplier (100 is default)
//------------------------------------------------------------------------------------------
void ServoDriver::GPSetSpeedMultiplyer(short sm) { _sGPSM = sm; }
...
void ServoDriver::BeginServoUpdate(void) {   // Start the update
  ...
  if (ServosEnabled) {
    if (g_fAXSpeedControl) {
      ...
      for (byte i = 0; i < NUMSERVOS; i++) g_awCurAXPos[i] = g_awGoalAXPos[i];
      #endif
    } else bioloid.interpolateStep(true);    // Make sure we call at least once
  }
}
...
void ServoDriver::OutputServoInfoForLeg(byte LegIndex, short sCoxaAngle1, short sFemurAngle1, short sTibiaAngle1) {
  ...
  if (ServosEnabled) {
    if (g_fAXSpeedControl) {
      ...
      if ((byte)pgm_read_byte(&cTarsLength[LegIndex])) bioloid.setNextPose(pgm_read_byte(&cPinTable[FIRSTTARSPIN + LegIndex]), wTarsSDV);
      #endif
    }
  }
  #ifdef DEBUG_SERVOS
...
word CalculateAX12MoveSpeed(word wCurPos, word wGoalPos, word wTime) {
  ...
  if ( wGoalPos > wCurPos) wTravel = wGoalPos - wCurPos;
  else wTravel = wCurPos - wGoalPos;
  ...
  if (wSpeed > 1023) wSpeed = 1023; // if the desired speed exceeds the maximum, we need to adjust
  if (wSpeed < 26) wSpeed = 26;     // we also use a minimum speed of 26 (5% of 530 the max value for 59RPM)
  return wSpeed;
}
#endif
...
void ServoDriver::CommitServoDriver(word wMoveTime) {
  ...
  if (ServosEnabled) {
    if (g_fAXSpeedControl) {
      ...
      setRX(0);
      #endif
    } else bioloid.interpolateSetup(wMoveTime);
  }
  #ifdef DEBUG_SERVOS
...
void ServoDriver::FreeServos(void) {
  if (!g_fServosFree) {
    ...
    #if 0
    for (byte i = 0; i < NUMSERVOS; i++) Relax(pgm_read_byte(&cPinTable[i]));
    #endif
    g_InputController.AllowControllerInterrupts(true);
    g_fServosFree = true;
  }
...
void MakeSureServosAreOn(void) {
  if (ServosEnabled) {
    if (!g_fServosFree) return;    // we are not free
    g_InputController.AllowControllerInterrupts(false);    // If on xbee on hserial tell hserial to not processess...
    if (g_fAXSpeedControl) {
      for (int i = 0; i < NUMSERVOS; i++) {
    ...
    else bioloid.readPose();

    SetRegOnAllServos(AX_TORQUE_ENABLE, 1);  // Use sync write to do it.

    #if 0
    for (byte i = 0; i < NUMSERVOS; i++) TorqueOn(pgm_read_byte(&cPinTable[i]));
    #endif
    g_InputController.AllowControllerInterrupts(true);
    g_fServosFree = false;
  }
...
void ServoDriver::BackgroundProcess(void) {
  if (g_fAXSpeedControl) return;  // nothing to do in this mode...
  if (ServosEnabled) {
    DebugToggle(A3);
    ...
    #ifdef cTurnOffVol          // only do if we a turn off voltage is defined
    #ifndef cVoltagePin         // and we are not doing AtoD type of conversion...
    // At least 4ms until next interpolation.  See how this works...
    if (iTimeToNextInterpolate > VOLTAGE_MIN_TIME_UNTIL_NEXT_INTERPOLATE ) GetBatteryVoltage();
    #endif
    #endif
  }
}
...
void TCTrackServos() {
  ...
  while(!DBGSerial.available()) {
    fChange = false;
    for(int i = 0; i < NUMSERVOS; i++) {
      uPos = ax12get(pgm_read_byte(&cPinTable[i]));
      // Lets put in a littl delta or shows lots
      if (abs(auPos[i] - uPos) > 1) {
        auPos[i] = uPos;
        if(fChange) printDebug(", ");
        else fChange = true;
        printDebug(pgm_read_byte(&cPinTable[i]), DEC);
        printDebug(": ");
        printDebug(uPos, DEC);

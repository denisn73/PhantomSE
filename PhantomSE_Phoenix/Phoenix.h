...
#ifdef USE_PHOENIX_COMMANDER

struct joyStruct {
  byte rcData[11];
  byte PhoenixCommanderConnected = false;
} command;

unsigned long lifePrevMillis = 0;
unsigned long BPS       = 0;
bool PhoenixHostConnected = false;

WiFiClient PhoenixClient;

bool PhoenixConnect(WiFiClient* _client, const char* host, int port) {
  printDebug("Connecting to Hexapod... ");
  bool state = false;
  if(_client->connect(host, port)) state = true; 
  if(!state) printlnDebug("failed!");
  else {
    MSound(3, 60, 2000, 80, 2250, 100, 2500);
    printlnDebug("OK!");
  }
...
bool PhoenixSend(WiFiClient* _client, byte* data, size_t _size) {
  size_t s = _size + 1500;
  uint8_t sbuf[s];
  for(int i=0; i<_size; i++) sbuf[i] = data[i];
  for(int i=_size; i<s; i++) sbuf[i] = i/10;
  bool state = true;
  BPS += 1500;
  _client -> setNoDelay(1);
  _client -> write(sbuf, s);
  yield();
  return state;
}
...
int PhoenixReadWire() {
  ...
  if(millis() - prevI2Ctest >= 1000) {
    ...
    if(Wire.endTransmission()==0) {
      if(!command.PhoenixCommanderConnected) {
        command.PhoenixCommanderConnected = true;
        printlnDebug("PhoenixCommander connected");
      }
    } else if(command.PhoenixCommanderConnected) {
      command.PhoenixCommanderConnected = false;
  ...
  if(command.PhoenixCommanderConnected) {
    if(millis() - prevI2Creq >= 50) {
      Wire.requestFrom(PhantomSE_commander_addr, 11);
      while(Wire.available()) {
        byte d = Wire.read();
        if(index<11) {
          command.rcData[index] = d;
          index++;
        }
      }
      prevI2Creq = millis();
...
void PhoenixCMD_loop(void) {
  if(WiFi.status() == WL_CONNECTED) {
    if(!PhoenixClient.connected()) PhoenixConnect(&PhoenixClient, PhoenixHost, PhoenixPort);
    else {
      if(PhoenixReadWire()) PhoenixSend(&PhoenixClient, command.rcData, sizeof(command.rcData));
    }
  }
  if(millis() - SecMillis >= 1000) {
    printDebug("BPS: ");
    printlnDebug(BPS*sizeof(command.rcData));
    BPS = 0;
    SecMillis = millis();
  }
  if(millis() - lifePrevMillis >= 10000) {
